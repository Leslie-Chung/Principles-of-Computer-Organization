 ...
 */
make_DHelper(E2G) {
  decode_op_rm(eip, id_src, true, id_dest, true);// dest(G)的值需要在译码时装入
}

make_DHelper(mov_E2G) {
  decode_op_rm(eip, id_src, true, id_dest, false);// dest(G)的值不需要在译码时装入
}
 ...
 * eAX <- Iv
 */
make_DHelper(I2a) { // imm --> eax/ax/al
...
/* used by unary operations 单项操作*/
/*对于Intel来说，第一个是目的操作数，后面的是源操作数*/
make_DHelper(I) {
